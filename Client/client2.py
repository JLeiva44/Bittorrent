...
class Client:
    ...
    def connect_to_peer(self,peer_ip, peer_port):
        try:
            ...
            # Realiza operaciones con client_socket aquí
            client_socket.send_json({"action": "ping"})
            response = client_socket.recv_json()
            logger.debug(f"Response from peer {peer_ip}:{peer_port}: {response}")
        except zmq.ZMQError as e:
            logger.error(f"ZMQError connecting to {peer_ip}:{peer_port} - {e}")
        finally:
            client_socket.close()
